...
Let the filter $c$ have length $M_1$ and an added delay equal to $D$, and the feedback filter $b$ have length $M_2$. Then the LE can be seen as a DFE with $M_2 = 0$. The filter coefficients are indeed computed in the same way, using the Wiener-Hopf theory and the MSE criterion applied to $J = E[|a{k-D} - y_k|^2]$. Note that we compare $a_{k-D}$ with $y_k$ because of the delay D introduced, therefore at the time $k$ at detection point the filter decides on $a_{k-D}$. The final equations that are implemented in the \texttt{DFE\_filter} function are the ones that solve the Wiener-Hopf equation, in particular
\begin{equation}
...
In order to visualize the equalization brought by these filter in Figures~\ref{fig:hledfe},~\ref{fig:LEcoeff} and~\ref{fig:DFEcoeff} there is the plot of the estimated impulse response $h_i$, both for LE and DFE, the plot of the feed-forward filter $c$ in the two cases, of the convolution between $h_i$ and $c$, which is $\psi_i = (h*c)_i$, and only for the DFE the plot if the coefficients of the feedback filter $b$. Note that the LE should try to equalize both postcursors and precursors, but in this case only the first are present. 
