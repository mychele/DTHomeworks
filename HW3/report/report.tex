...
The Viterbi algorithm was implemented as a MATLAB function that performs the maximum likelihood detection using the whole message, including the PN preamble for channel estimation. Therefore, the initial conditions are given by the final symbols of the training sequence, that is hence used for the training of the Viterbi algorithm. The parameters $L_1 = 0$ and $L_2 = 4$ are set equal to $N_1$ and $N_2$ from problem 1.

The state of the FSM associated with the algorithm at time $k$ is as in \cite{bc}:
\begin{equation}
\mathbf{s}_k = (a_{k+L1}, \ldots, a_{k-L_2+1}) = (a_k, \ldots, a_k-3), \quad k=0...K-1,
\end{equation}
...
\begin{equation}
s_k = 1 + \sum_{i=0}^{L_1+L_2-1} M^i \alpha_{k+L_1-i} = 1 + \sum_{i=0}^{3} M^i \alpha_{k-i},
\end{equation}
where 1 was added in order to have $s_k \in \{1, \ldots, N_S\}$.
At time $k+1$, given the state $\mathbf{s}_k$ represented by $s_k$, and assuming the new symbol is $a_{k+1}$, the new state index is obtained by setting to 0 the two most significant bits (corresponding to the oldest symbol), shifting left by 2 bits and then adding the index of the new symbol in the two least significant bits:
\begin{equation}
s_{k+1} = \left[ (s_k - 1)_{\mathrm{mod} M^{L1+L2-1}} \right] M + \alpha_{k+1} + 1 = \left[ (s_k - 1)_{\mathrm{mod} \frac{N_S}{M}} \right] M + \alpha_{k+1} + 1.
\end{equation}
In our implementation, we check all possible transitions by cycling through all state indices $s_k$, and for each of them considering the $M=4$ new possible symbols. Performing this iteration, the resulting new state index
