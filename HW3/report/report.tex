...
In particular, in order to estimate as first the length of the impulse response and $N_1, N_2$, we operate as follows. Let $N_{max} = N_{seq} = 10$ the maximum possible length of the impulse response. As first, we set $N_1 = 0$ and compute the functional of the error 
\begin{equation} %% TODO define N
	\mathcal{E} = \frac{1}{L}\sum_{k = N - 1}^{(L-1)+(N-1)}|d(k)-\hat{d}(k)|^2
	\label{eq:functional}
\end{equation}
