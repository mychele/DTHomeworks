...
Tc = 1;
T = 4 * Tc;
snr_vec_fba_513 = 5 : 13; % dB
L_data = 2.^[15 15 15 15 18 18 20 20 22] - 1;
...
for snr_i = 1:length(snr_vec_fba_513)
    thissnrstart = tic;
    snr_curr = snr_vec_fba_513(snr_i);
    parfor sim = 1:numsim     % perform sim_each simulations
        ...
        pbit_fba_513(snr_i, sim) = pbit_this;
        n_biterr_fba_513(snr_i, sim) = n_biterr_this;
    end
    
    fprintf('SNR=%ddB completed in %.2f minutes\n', snr_vec_fba_513(snr_i), toc(thissnrstart)/60)
end

delete(gcp);

save('pbit_fba_513', 'pbit_fba_513', 'n_biterr_fba_513', 'snr_vec_fba_513');