...
Tc = 1;
T = 4 * Tc;
snr_fba_real_channel = 12; % dB
L_data = 2.^22 - 1;
numsim = 4;

pbit_fba_real_channel = zeros(numsim);
n_biterr_fba_real_channel = zeros(numsim);
...
for sim = 1:numsim
    ...
    [packet, r, sigma_w] = txrc(L_data, snr_curr, assumed_m_opt);
    ...
    pbit_fba_real_channel(sim) = pbit_this;
    n_biterr_fba_real_channel(sim) = n_biterr_this;
    
    save(strcat('pbit_fba_12db_', num2str(sim)), 'pbit_fba_real_channel', 'n_biterr_fba_real_channel');    
    
    fprintf('SNR=%ddB completed in %.2f minutes\n', snr_fba_real_channel, toc(thissnrstart)/60)
end
