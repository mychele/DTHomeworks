...
num_bit_errorDFE = zeros(length(snr_vec), sim_each, length(M1_vec));
...
for snr_i = 1:length(snr_vec)
    ...
    for m_ind = 1:length(M1_vec)
        
        N = N1+N2+1;    % For each symbol, we have N-1 interferers + the symbol
        M1_dfe = M1_vec(m_ind);        % FF filter: equal to the span of h
        D_dfe = M1_dfe-1;
...
save('pbit_DFE', 'pbitDFE', 'num_bit_errorDFE', 'snr_vec', 'M1_vec');
