...
\end{equation}
where $C = \sqrt{K / (K+1)}$ is the deterministic line of sight (\textit{LOS}) component of the first ray (for $i=0$) and $K$ is the provided Rice factor. In order to compute the infinite sum above, only the initial 900 samples are actually used, as MATLAB approximates all $\M(iT_C)$ values for $i > 895$ to zero.
...
The histogram can be seen in Fig.~\ref{fig:h1hist1000realizations}.
