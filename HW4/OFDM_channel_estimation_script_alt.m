...
for sim=1:numsim
   ...
   est_sigma_w_new(sim) = 0;
   for j=0:nsamples-1
      tempGest = G_est((j*symbpersegment + 1) : (j*symbpersegment + 1)+3);
      est_sigma_w_new(sim) = est_sigma_w_new(sim) + ...
         var(tempGest - mean(tempGest));
   end
   est_sigma_w_new(sim) = est_sigma_w_new(sim) / nsamples / M * sigma_ts;
...
figure,
errorbar([mean(est_sigma_w), mean(est_sigma_w_new)], ...
   [1.96*std(est_sigma_w)/sqrt(numsim), ...
    1.96*std(est_sigma_w_new)/sqrt(numsim)], '.')
hold on
plot([.5 2.5], sigma_w * [1 1])
hold off
grid on
