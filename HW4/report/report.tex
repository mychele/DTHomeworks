...
If we set the DFE to use the actual impulse response of the channel and channel noise, we can assume that there is no ISI left after the cancellation performed using the $b$ filter. Thus, the variance of the noise component of the samples $y_k = y_{k,I} + j y_{k,Q}$ considered in the LLR formula is $\sigma_w^2 = 2 \sigma_I^2$, as only the white component of the noise is left. As the only noise that we assume to be present is the channel noise, the LLR formula that is used in this case is the same as that used in Problem 1:

\begin{equation}
	\ell'_p =
	\begin{dcases}
	\frac{2 y_{k,I}}{\sigma_I^2}, \quad p = 2k \\
	\frac{2 y_{k,Q}}{\sigma_I^2}, \quad p = 2k +1
	\end{dcases}
\end{equation}

When, on the other hand, at the DFE we use an estimate of the channel impulse response and channel noise, we assume that the ISI cancellation may not be perfect. In this case, the expression for the samples at detection point takes into account the noise variance computed in~\eqref{eq:noisevar}, and hence becomes:
\begin{equation}
	\ell'_p =
	\begin{dcases}
...
\end{equation}
differently from the equally spaced pattern defined in \eqref{eq:OFDMequallyspacedindices}. The frequency response estimation is illustrated in Fig.~\ref{fig:p3_newmethodexample}, in which we show the true response $\mathbf{G}_c$, the estimate $\mathbf{\hat{G}}_c$, and the first raw estimate $\mathbf{\tilde{G}}_c$. The latter vector is shorter then the other, since it consists of $L_{ts}$ elements, which are unevenly placed across the frequency spectrum as per \eqref{eq:OFDMnewmethodindices}.
...
\begin{equation}
	\hat{\sigma}_{w,q}^2 = v_q \: \dfrac{\sigma_{ts}^2}{\ofdM}.
\end{equation}
Since we have 32 allowed subchannel, and we used the scheme defined in \eqref{eq:OFDMnewmethodindices} for the frequency response estimation, we can iterate the noise estimation on all 8 groups and then take the average, so the estimate of the noise variance is finally
\begin{equation}
